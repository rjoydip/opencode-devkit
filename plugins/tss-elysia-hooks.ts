/**
 * OpenCode Devkit Plugin Hooks for OpenCode
 *
...
export const TEHooksPlugin = async ({ client, $, directory, worktree }: PluginInput) => {
  ...
  return {
    ...
    "file.edited": async (event: { path: string }) => {
      ...
      if (hookEnabled("post:edit:format", ["strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          log("info", `[OpenCode Devkit] Formatted: ${event.path}`);
        } catch {
      ...
      if (
        ...
        try {
          ...
          if (result.trim()) {
            ...
            log(
              "warn",
              `[OpenCode Devkit] console.log found in ${event.path} (${lines} occurrence${lines > 1 ? "s" : ""})`,
            );
          }
        } catch {
    ...
    "tool.execute.after": async (input: { tool: string; args?: { filePath?: string } }) => {
      ...
      if (
        ...
        try {
          ...
          log("info", "[OpenCode Devkit] TypeScript check passed");
        } catch (error: unknown) {
          const err = error as { stdout?: string };
          log("warn", "[OpenCode Devkit] TypeScript errors detected:");
          if (err.stdout) {
      ...
      if (
        ...
        log("info", "[OpenCode Devkit] PR created - check GitHub Actions status");
      }
    },
    ...
    "session.created": async () => {
      ...
      log("info", `[OpenCode Devkit] Session started - profile=${currentProfile}`);

      // Check for project-specific context files
      try {
        const hasOpencodeMd = await $`test -f ${worktree}/OPENCODE.md && echo "yes"`.text();
        if (hasOpencodeMd.trim() === "yes") {
          log("info", "[OpenCode Devkit] Found OPENCODE.md - loading project context");
        }
      } catch {
    ...
    "session.idle": async () => {
      ...
      log("info", "[OpenCode Devkit] Session idle - running console.log audit");
      ...
      if (totalConsoleLogCount > 0) {
        log(
          "warn",
          `[OpenCode Devkit] Audit: ${totalConsoleLogCount} console.log statement(s) in ${filesWithConsoleLogs.length} file(s)`,
        );
        filesWithConsoleLogs.forEach((f) => log("warn", `  - ${f}`));
        log("warn", "[OpenCode Devkit] Remove console.log statements before committing");
      } else {
        log("info", "[OpenCode Devkit] Audit passed: No console.log statements found");
      }
    ...
    "session.deleted": async () => {
      ...
      log("info", "[OpenCode Devkit] Session ended - cleaning up");
      editedFiles.clear();
    },
    ...
    "todo.updated": async (event: { todos: Array<{ text: string; done: boolean }> }) => {
      ...
      if (total > 0) {
        log("info", `[OpenCode Devkit] Progress: ${completed}/${total} tasks completed`);
      }
    },
    ...
    "experimental.session.compacting": async () => {
      const contextBlock = [
        "# OpenCode Devkit Context (preserve across compaction)",
        "",
        "## Active Plugin: OpenCode Devkit v0.0.0",
    ...
    "permission.ask": async (event: { tool: string; args: unknown }) => {
      log("info", `[OpenCode Devkit] Permission requested for: ${event.tool}`);
