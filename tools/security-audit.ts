 ...
 * Combines bun audit, secret scanning, and OWASP checks.
 *
  ...
  async execute(args, context) {
    ...
    if (auditType === "all" || auditType === "dependencies") {
      results.checks.push({
        ...
        command: fix ? "bun audit fix" : "bun audit",
...
function generateRecommendations(results: AuditResults): string[] {
  ...
  for (const check of results.checks) {
    ...
    if (check.status === "pending" && check.name === "Dependency Vulnerabilities") {
      recommendations.push("Run 'bun audit' to check for dependency vulnerabilities");
      recommendations.push("Consider using 'bun audit fix' to auto-fix issues");
    }
  }
